...
# https://argocd.dev44-just-homestyle.duckdns.org/
module "oidc_client" {
  source = "../../../keycloak/configuration/modules/oidc-client"

  realm_id    = var.realm_id
  client_id   = "argocd"
  root_url    = "https://argocd.dev44-just-homestyle.duckdns.org"
  base_url    = "/applications"
  web_origins = [""]

  admin_url = "https://argocd.dev44-just-homestyle.duckdns.org"
  redirect_uris = [
    format("https://%s/auth/callback", var.argocd_fqdn),
    format("https://%s/*", var.argocd_fqdn)
  ]
}
...
resource "keycloak_role" "reader_role" {
  ...
  client_id   = module.oidc_client.this.id
  name        = "argocd-reader"
  description = "ArgoCD Reader role"
}
